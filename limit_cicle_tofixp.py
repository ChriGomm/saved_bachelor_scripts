...
def stab_find(kval,wval,dval):
    ...
    for i,s in enumerate(start):
        traj = solve_ivp(gl2,(0,1000),y0=s,args=arguments,method='LSODA',t_eval=np.arange(numb_of_tsteps)/numb_of_tsteps*1000)#,rtol=1e-12,atol=1e-25)
...
w_cut_traj = np.zeros_like(k_cut_traj)
d_cut_traj = np.zeros_like(k_cut_traj)
# result = po.imap(endp_wrapper,range(gridpoints),chunksize=10)
...
# np.save(root_path+'k_cutarray3',k_cutarray)
# np.save(root_path+'w_cutarray3',w_cutarray)
# np.save(root_path+'d_cutarray3',d_cutarray)
# np.save(root_path+'k_cut_traj',k_cut_traj)
# np.save(root_path+'w_cut_traj',w_cut_traj)
# np.save(root_path+'d_cut_traj',d_cut_traj)
duration = time()-t1

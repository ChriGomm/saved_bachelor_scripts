...
gridpoints = 54
w_cut_array_old = np.load(root_path+'w_cut_aver_sorted.npy')
number_c = np.load(root_path+'w_cut_number_of_lc.npy')
...
def stab_find(kval,wval,dval):
    ...
    trajs = []
    start = starting_points(mgrid,mgrid,mgrid)
...
# w_cut_aver[:,:,:,:15] = w_cut_array_old
w_cut_aver = np.load(root_path+f'w_cut_aver_p_corrected.npy')
...
for gridx in range(2,3):
    ...
    # for gridy in range(3):
        w_cut_traj = np.load(root_path+f'w_cut_traj_p_corrected_row{gridx}.npz')['arr_0']
        for i in range(subgrid):
            for j in range(gridpoints):
                if area_select(i,j):
                    for n in range(numb_of_start):
                        w_cut_aver[gridx*subgrid+i,j,:,n], trash = average(w_cut_traj[i,j,n,:,1:].T)
        # po = Pool(9)
        ...
        # result = po.imap(endp_wrapper,np.reshape(i_mat,(subgrid**2)),chunksize=10)
        # for index, res in enumerate(result):
        #     l, m = index//subgrid, index%subgrid
        #     ind = int(i_mat[l,m])
    ...
    # np.savez_compressed(root_path+f'w_cut_traj_p_corrected_row{gridx}',w_cut_traj)
        ...
        duration = time()-t1
        print(f"{duration//3600}h, {(duration%3600)//60}min, {duration%60}s, {gridx}")

# np.save(root_path+f'w_cut_peval_corrected',period_eval)
np.save(root_path+f'w_cut_aver_p_corrected2',w_cut_aver)
print('finished')